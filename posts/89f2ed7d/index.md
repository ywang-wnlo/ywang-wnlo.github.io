...
然而一般的程序输出可能不足以提供足够信息的情况，此时可以根据 `segmentation fault` 信息并且借助一些工具来定位出错的位置
...
$ ./test
[1]    7533 segmentation fault  ./test
...
# 运行程序，会出现 segmentation fault
$ ./test
[1]    8173 segmentation fault  ./test
...
当程序发生崩溃时，若已启用 `coredump` 功能，系统会生成包含完整内存信息的 `core` 文件。该文件记录了程序崩溃时的寄存器状态、堆栈信息、内存内容等关键调试数据
...
# 默认的 core 文件保存在程序的当前目录下，名称为 core
$ cat /proc/sys/kernel/core_pattern
core

# 自定义 core 文件的保存位置和名称
# %e 表示程序名，%p 表示 PID，%t 表示时间戳
...
依然用上面的程序来测试，运行后，会生成一个 `core` 文件，之后可以用 `gdb` 分析

```bash
$ ./test arg1
[1]    14724 segmentation fault (core dumped)  ./test arg1
```

使用 `gdb` 分析 `core` 文件的基本流程：
...
[New LWP 14724]
Core was generated by `./test arg1&#39;.
Program terminated with signal 11, Segmentation fault.
#0  0x000000000040053d in func_bug () at test.c:5
5           *p = 2;
Missing separate debuginfos, use: debuginfo-install bash-4.2.46-34.tl2.3.x86_64 glibc-2.17-326.tl2.3.x86_64
(gdb) bt
#0  0x000000000040053d in func_bug () at test.c:5
#1  0x0000000000400584 in main (argc=2, argv=0x7ffe45a41ed8) at test.c:13
(gdb) l
1       #include &lt;stdio.h&gt;
2
3       void func_bug() {
...
1. 崩溃发生在 test.c 第 5 行的空指针写操作
2. 函数调用栈显示由 main 函数第 10 行触发
3. 变量 p 的值为 0x0（NULL）
...
- [【man】core](https://www.man7.org/linux/man-pages/man5/core.5.html)
